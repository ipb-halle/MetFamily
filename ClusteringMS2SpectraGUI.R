...
#########################################################################################
## helper
analyzeTreeFromRoot <- function(cluster, featureMatrix, filter){
  ...
  #innerNodeFeaturesBinaryHere <<- list()
  innerNodeFeaturesHere <<- list()
  innerNodeFeaturesCountsHere <<- list()
  innerNodeFeaturesMeanAbundanceHere <<- list()
  innerNodeFeaturesCounterHere <<- list()
  ...
  analyzeTree(cluster, featureMatrix, filter, rootIndex)
  
  ## box
  resultObj <- list()
  resultObj$innerNodeMembersTree <- innerNodeMembersTreeHere
  resultObj$innerNodeMembers <- innerNodeMembersHere
  #resultObj$innerNodeFeaturesBinary <- innerNodeFeaturesBinaryHere
  resultObj$innerNodeFeatures <- innerNodeFeaturesHere
  ...
  resultObj$innerNodeFeaturesCounter <- innerNodeFeaturesCounterHere
  resultObj$innerNodePosition <- innerNodePositionHere
  resultObj$leafHeights <- leafHeightsHere
  
  return(resultObj)
}
analyzeTree <- function(cluster, featureMatrix, filter, nodeIdx){
  if(nodeIdx < 0){
    ###################################
    ## leaf
    leafIdx <- -nodeIdx
    precursorIndex <- filter[leafIdx]
    featureValues <- featureMatrix[precursorIndex, ]
    ...
    resultObj$featuresBinary <- featuresBinary
    ...
    resultObj$featuresCounter <- featuresCounter
    ...
    ###################################
    ## inner node
    resultObj.l <- analyzeTree(cluster, featureMatrix, filter, cluster$merge[nodeIdx, 1])
    resultObj.r <- analyzeTree(cluster, featureMatrix, filter, cluster$merge[nodeIdx, 2])
    
    membersTree <- c(resultObj.l$membersTree, resultObj.r$membersTree)
    members <- c(resultObj.l$members, resultObj.r$members)
    featuresBinary <- resultObj.l$featuresBinary & resultObj.r$featuresBinary
    ...
    innerNodeFeaturesHere[[nodeIdx]] <<- resultObj$features
    ...
    innerNodeFeaturesCounterHere[[nodeIdx]] <<- resultObj$featuresCounter
...
getSetOfSubTreesFromRoot <- function(dataList, clusterFilter, clusterCluster, fragmentMass){
  rootIndex <- length(clusterCluster$cluster$height)
  result <- getSetOfSubTrees(dataList, clusterFilter, clusterCluster, fragmentMass, rootIndex)
  if(result$comprisesFragmentMass)
    result$results <- c(result$results, rootIndex)
  return(result$results)
}
getSetOfSubTrees <- function(dataList, clusterFilter, clusterCluster, fragmentMass, index){
  if(index<0){ # it is a leaf
    leafIndex <- -index
    precursorIndex <- clusterFilter$filter[[leafIndex]]
    ...
    result <- list()
    result$results <- c()
    result$comprisesFragmentMass <- comprisesFragmentMass
    
    #print(paste("+++++++++++++++++", index, precursorIndex, dataList$precursorLabels[[precursorIndex]], comprisesFragmentMass))
    
    return(result)
  }
  
  result.l  <- getSetOfSubTrees(dataList, clusterFilter, clusterCluster, fragmentMass, clusterCluster$cluster$merge[index, 1])
  result.r  <- getSetOfSubTrees(dataList, clusterFilter, clusterCluster, fragmentMass, clusterCluster$cluster$merge[index, 2])
  
  #print(paste("#", index, clusterCluster$cluster$merge[index, 1], clusterCluster$cluster$merge[index, 2], result.l$comprisesFragmentMass, result.r$comprisesFragmentMass))
  
  result <- list()
  result$results <- c(result.l$results, result.r$results)
  if(result.l$comprisesFragmentMass & result.r$comprisesFragmentMass){
    ## both children do comprise the fragment
    result$comprisesFragmentMass <- TRUE
  } else {
    result$comprisesFragmentMass <- FALSE
    if((!result.l$comprisesFragmentMass & result.r$comprisesFragmentMass) | (result.l$comprisesFragmentMass & !result.r$comprisesFragmentMass)){
      ## only one child does comprise the fragment
      if(result.l$comprisesFragmentMass)
        result$results <- c(result$results, clusterCluster$cluster$merge[index, 1])
      else
        result$results <- c(result$results, clusterCluster$cluster$merge[index, 2])
    } else {
...
readClusterData <- function(file, progress = FALSE){
  ...
  dataColumnsNameFunction <- function(group){
    paste(as.character(group), "_", columnNames[groupsStartEndMatrix[group, 1]:groupsStartEndMatrix[group, 2]], sep = "")
  }
  for(groupIdx in 1:length(groups)){
    dataColumnNames <- dataColumnsNameFunction(groups[[groupIdx]])
  ...
  dataMeanColumnNameFunction <- function(group){
    paste(as.character(group), "_mean", sep = "")
  }
  dataMeanColumnNames <- list()
  for(groupIdx in 1:length(groups)){
    dataColumnName <- dataMeanColumnNameFunction(groups[[groupIdx]])
  ...
  for(groupIdx1 in 1:length(groups))
    for(groupIdx2 in 1:length(groups)){
      ...
      dataFrameMeasurements[, lfcColumnName] <- log(
        x = dataFrameMeasurements[, dataMeanColumnNameFunction(group = groups[[groupIdx2]])] / dataFrameMeasurements[, dataMeanColumnNameFunction(group = groups[[groupIdx1]])], 
        base = 2
      )
  ...
  columnGroupLabels <- sapply(X = groups, FUN = function(x){ rep(x = x, times = length(dataColumnsNameFunction(x))) })
  ...
  dataList$dataColumnsNameFunction <- dataColumnsNameFunction
  dataList$dataMeanColumnNameFunction <- dataMeanColumnNameFunction
...
filterData <- function(dataList, groups, filter_average, filter_lfc, filterList_ms2_masses, filter_ms2_ppm, progress = FALSE){
  ##########################################
  ## filter
  ...
  if(!is.null(filter_average))
    filter <- filter & apply(dataList$dataFrameMeasurements[, sapply(X = groups, FUN = dataList$dataMeanColumnNameFunction)], 1, mean) >= filter_average
  
  ## filter_lfc
  if(!is.null(filter_lfc)){
    if(filter_lfc != 0){
      if(filter_lfc > 0)
        filter <- filter & dataList$dataFrameMeasurements[, dataList$lfcColumnNameFunction(groups[[1]], groups[[2]])] >= filter_lfc
      else
        filter <- filter & dataList$dataFrameMeasurements[, dataList$lfcColumnNameFunction(groups[[1]], groups[[2]])] <= filter_lfc
    }
  }
...
calculateCluster <- function(dataList, filter, distanceMatrix, method, progress = FALSE){
  ...
  cluster <- hclust(d = stats::as.dist(m = distanceMatrix), method = method)
  ...
  resultObj <- analyzeTreeFromRoot(cluster = cluster, featureMatrix = dataList$featureMatrix, filter)
  ...
  innerNodeFeatures <- resultObj$innerNodeFeatures
  ...
  innerNodeFeaturesCounter <- resultObj$innerNodeFeaturesCounter
  ...
  text <- unlist(c(innerNodeFeaturesCounter, apply(X = dataList$featureMatrix[filter, ], MARGIN = 1, FUN = function(x) sum(x != 0))))
  ...
  filterList$innerNodeFeatures <- innerNodeFeatures
  ...
  filterList$innerNodeFeaturesCounter <- innerNodeFeaturesCounter
...
getMS2spectrum <- function(dataList, filterList, label){
  if(label < 0){
    ...
    ## inner node
    clusterIndex <- label
    clusterMembers <- sort(filterList$innerNodeMembers[[clusterIndex]])
    
    #features <- filterList$innerNodeFeaturesBinary[[clusterIndex]]
    features <- filterList$innerNodeFeatures[[clusterIndex]]
    fragmentsX <- dataList$fragmentMasses[features]
...
getMS2spectrumOfPrecursor <- function(dataList, precursorIndex){
  features <- which(dataList$featureMatrix[precursorIndex, ] != 0)
  featureMasses <- dataList$fragmentMasses[features]
  featureValues <- dataList$featureMatrix[precursorIndex, features]
...
calcClusterPlotDendrogram <- function(filterList, distanceMeasure, nodeIndex = NULL, nodeIndeces = NULL, xInterval = NULL){
  if(is.null(xInterval))
    xInterval <- c(1, filterList$numberOfPrecursorsFiltered)
  ...
  if(rightMostInvisibleLabelIndex > 0){
    labelsToRemove <- filterList$cluster$labels[filterList$cluster$order][1:rightMostInvisibleLabelIndex]
    
    colLab <- colorLabels(filterList$cluster$labels, NULL, NULL, labelsToRemove)
    dend <- dendrapply(dend, colLab)
  }
  
  ## color labels for multiple sub-roots
  if(!is.null(nodeIndeces)){
    clusterMembers <- c(unlist(filterList$innerNodeMembersTree[nodeIndeces[nodeIndeces > 0]]), -nodeIndeces[nodeIndeces < 0])
    
    colLab <- colorLabels(filterList$cluster$labels, clusterMembers, 'green')
    dend <- dendrapply(dend, colLab)
  }
  ## color labels for single sub-root
  if(!is.null(nodeIndex)){
    if(nodeIndex > 0){
      clusterMembers <- filterList$innerNodeMembersTree[[nodeIndex]]
    } else {
      clusterMembers <- c(-nodeIndex)
    }
    
    colLab <- colorLabels(filterList$cluster$labels, clusterMembers, 'blue')
    dend <- dendrapply(dend, colLab)
  }
  ...
  if(!is.null(nodeIndeces)){
    for(nodeIdx in nodeIndeces[nodeIndeces > 0]){
      ## sub tree coloring
      clusterMembers <- filterList$innerNodeMembersTree[[nodeIdx]]
      a2r_counter <<- min(match(x = clusterMembers, table = filterList$cluster$order)) - 1
      colorSubTree(cluster = filterList$cluster, index = nodeIdx, col = "green")
    }
  }
  ...
  ## POI's
  points(x = filterList$poiCoordinatesX, y = filterList$poiCoordinatesY, col = "red", pch=19, cex=1.)
  graphics::text(  x = filterList$poiCoordinatesX, y = filterList$poiCoordinatesY + 0.02, labels = filterList$poiText, pos = 4)
}
calcClusterPlotHeatmap <- function(dataList, filter, filterList, xInterval = NULL){
  if(is.null(xInterval))
    xInterval <- c(1, filterList$numberOfPrecursorsFiltered)
  ...
  ## heatmap
  columnsOfInterest <- c(
    dataList$dataMeanColumnNameFunction(filter$groups[[1]]), dataList$dataMeanColumnNameFunction(filter$groups[[2]]), 
    dataList$lfcColumnNameFunction(groupOne = filter$groups[[1]], groupTwo = filter$groups[[2]])
  )
  ...
  if(length(columnsOfInterest) == 0){
    ...
    colorOne <- dataList$colorMatrixDataFrame[filterList$filter, columnsOfInterest[[1]]][filterList$cluster$order]
    colorTwo <- dataList$colorMatrixDataFrame[filterList$filter, columnsOfInterest[[2]]][filterList$cluster$order]
    colorLFC <- dataList$colorMatrixDataFrame[filterList$filter, columnsOfInterest[[3]]][filterList$cluster$order]
    
    plot(x = c(1, filterList$numberOfPrecursorsFiltered), y = c(0, 3), type= "n", xlab = "", ylab = "", axes = FALSE, xlim = xInterval, ylim = c(0, 3))
    for(i in 1:filterList$numberOfPrecursorsFiltered){
...
calcClusterPlotMS2 <- function(dataList, fragmentsX = c(), fragmentsY = c(), xInterval = NULL){
  ####################
  ## fragment spectrum
  ...
  plot(x = fragmentsX, y = fragmentsY, ylab = "Relative abundance", xlab = "", xlim = xInterval, ylim = c(0, 1.1), xaxt='n', yaxt='n')
  axis(side = 2, at = seq(from = 0, to = 1, by = 0.25), labels = c(0, "", 0.5, "", 1))
  axis(side = 3)
...
calcPCA <- function(dataList, filter, unitVariance, logTransform){
  dataFrame <- dataList$dataFrameMeasurements[filter$filter, sapply(X = filter$groups, FUN = dataList$dataColumnsNameFunction)]
  dataFrame <- t(dataFrame)
  
  if(logTransform){
    dataFrame <- log(dataFrame + 1)
  }
  
  ## TODO
  pca = PCA(dataFrame, graph = FALSE, scale.unit = unitVariance)
  
  ## individuals: pca3$ind$coord (scores)
  ## variables  : pca3$var$coord (loadings)
  
  return(pca)
}
plotPCAscores <- function(pca, dataList, filter, pcaDimensionOne, pcaDimensionTwo, xInterval = NULL, yInterval = NULL){
  #rPlot(Dim.1 ~ Dim.2, data = pca$var$coord[, c(1, 2)])
  
  palette <- colorPalette()
  colors <- palette[dataList$columnGroupLabels]
  ...
  plot(x = dataDimOne, y = dataDimTwo, xlim = xInterval, ylim = yInterval, xlab = xAxisLabel, ylab = yAxisLabel, main = "Scores", col = colors, pch=19, cex=0.7)
  ...
  ## data labels
  for(idx in 1:nrow(pca$ind$coord)){
    graphics::text(x = pca$ind$coord[idx, pcaDimensionOne], y = pca$ind$coord[idx, pcaDimensionTwo], labels = dataList$columnGroupLabels[!is.na(match(x = dataList$columnGroupLabels, table = filter$groups))][[idx]], pos = 4)
  }
}
plotPCAloadings <- function(pca, dataList, filter, pcaDimensionOne, pcaDimensionTwo, xInterval = NULL, yInterval = NULL){
  #rPlot(Dim.1 ~ Dim.2, data = pca$var$coord[, c(1, 2)])
  ...
  par(mar=c(5, 4, 4, 2) + 0.1, mgp = c(3, 1, 0))  ## c(bottom, left, top, right)
  plot(x = dataDimOne, y = dataDimTwo, xlim = xInterval, ylim = yInterval, xlab = xAxisLabel, ylab = yAxisLabel, main = "Loadings", pch=19, cex=0.7)
